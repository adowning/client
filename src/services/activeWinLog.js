...
/**
 * ActiveWinLog - stores the name of the timesheet that is being referenced
 * @constructor
 */

class ActiveWinLog {
  // Use a class because
  constructor(timesheet, profile) {
  ...
  async init() {
    // try {
    console.log('checking existinace')
    if (await parseApi.exists('DesktopActivity', 'timesheet', this.timesheet)) return
    console.log('does not exist')
    this.setLogData({ pid: process.pid, stats: {} })
    // } catch (err) {
